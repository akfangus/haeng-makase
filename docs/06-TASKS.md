...
- **Tasks:**
  ...
  - [x] 3. "장바구니를 입에 문 시바견 이미지를 플레이스홀더로 사용하여 로딩 컴포넌트 `LoadingShiba.tsx`를 구현하라."
  - [x] 4. "모든 페이지의 우측 하단(또는 헤더)에 '돈키호테 바로가기' 플로팅 버튼(펭귄 아이콘 등)을 구현하고, 클릭 시 `/donki` 페이지로 이동하도록 하라."
